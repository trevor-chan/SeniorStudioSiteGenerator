public class ImageManipulator{
  ...
  public ImageManipulator (PImage img, color seekcolor, int edgeEffect, int cm) {
    this.img = img;
    width_ = img.width;
    height_ = img.height;
    red = seekcolor >> 16 & 0xFF;
  ...
  /*public int[] greyScale() {
    ...
    for(int i = 0; i < img.pixels.length; i++) {                                      // Create a 'greyscale' image based on the difference from the r g b color
        if (colormetric ==0 ){
          ...
          result[i] = (int)((abs((img.pixels[i] >> 16 & 0xFF) - red)*0 + abs((img.pixels[i] >> 8 & 0xFF) - green)*0 
          + abs((img.pixels[i] & 0xFF) - blue))*0.33333); 
        }
    }
    return result;
  }*/
    ...
    for(int i = 0; i < img.pixels.length; i++) {                                      // Create a 'greyscale' image based on the difference from the r g b color
        if (colormetric ==0 ){
          result[i] = (float)((abs((img.pixels[i] >> 16 & 0xFF) - red) + abs((img.pixels[i] >> 8 & 0xFF) - green) + abs((img.pixels[i] & 0xFF) - blue))*.1);//.333
        }
        else{
          result[i] = (float)((abs((img.pixels[i] >> 16 & 0xFF) - red)*1 + abs((img.pixels[i] >> 8 & 0xFF) - green)*0 
          + abs((img.pixels[i] & 0xFF) - blue))*0); 
        }
    }
    return result;
  }
  
  private PVector calcSpot(int iCenter, float[] scale) {
